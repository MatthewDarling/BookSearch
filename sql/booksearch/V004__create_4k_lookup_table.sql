CREATE TABLE IF NOT EXISTS file_lookup_4k
...
CREATE INDEX "fl_4k_tsv_idx" ON file_lookup_4k USING GIN (content_tsv tsvector_ops);
...
CREATE OR REPLACE FUNCTION tfn_split_file_to_indices_to_4k() RETURNS trigger
AS $$
DECLARE num_spaces INT;
BEGIN
  DELETE FROM file_lookup_4k WHERE file_id = NEW.file_id;

  INSERT INTO file_lookup_4k (file_id, sequence_no, content_tsv, content_array, content)
  SELECT file_id, ROW_NUMBER() OVER (), to_tspvector(content), TO_TSP_TEXT_ARRAY(content), content
    FROM (SELECT NEW.file_id, LARGE_TEXT_TO_TSVECTORS('english', NEW.content, 12::SMALLINT) as content) AS frags
...
CREATE TRIGGER t_update_file_text_4k_tsv
AFTER INSERT OR UPDATE OF content ON files
FOR EACH ROW EXECUTE FUNCTION tfn_split_file_to_indices_to_4k();
